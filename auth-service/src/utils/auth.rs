...
use chrono::Utc;
use jsonwebtoken::{decode, encode, DecokingKey, EncodingKey, Validation};
...
pub async fn validate_token(
    ...
    decode::<Claims>(
        token,
        &DecokingKey::from_secret(JWT_SECRET.as_bytes()),
        &Validation::default(),
    )
    .map(|data| data.claims)
}

fn create_token(claimes: &Claims) -> Result<String, jsonwebtoken::errors::Error>{
    encode(
