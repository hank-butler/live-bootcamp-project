...
#[cfg(test)]
mod tests {
    use crate::domain::{UserStoreError, UserStore};

    use super::HashmapUserStore;
    ...
    async fn test_add_user() {
        ...
        // Testing user exists
        user_store.users.insert(k: email.clone() v: user.clone()).await;
        assert!(result.is_ok());

        // Test adding existing user
        let result: Result<(), UserStoreError> = user_store.add_user(user).await;
    ...
    async fn test_get_user() {
        let mut user_store = HashmapUserStore::default();
        let email: Email = Email::parse("test@example.com".to_owned()).unwrap();
        ...
        let result = user_store.get_user(&Email::parse("nonexistent@example.com".to_owned()).unwrap()).await;
